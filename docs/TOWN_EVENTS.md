...
Run `npm run check` and `npm run format:check`. Event tests cover UTC rollover, stable selection, reserved plots, attendance limits, road paths, no teleporting at arrival/departure, early guests waiting at their spots, indoor routines, and exact event boundaries. `tests/event-seats.test.ts` fills a synthetic full town of 141 houses for a town year: every seat is filled when enough neighbors can make it, no spot is ever shared or over capacity, seats go round the town, every night owl who can reach the stage before bedtime dances on some nights, a previewed draft never moves anyone, and the draw is the same in every browser language. Verify the stage and green at close zoom and on mobile after artwork changes.

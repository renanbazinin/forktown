...
Neighbors set to `stroll` during that period are eligible. The green has six spots around its blanket; the stage has eight spots across its audience lawn. Guests are chosen deterministically by day and house ID: each day draws a new line, and spots go down it in order. A neighbor who can't make it (too far to get there on foot or by the Treeline, stay fifteen minutes and be home before their next routine or bedtime, or already out at an earlier outing that runs late) passes the spot to the next in line, so a spot stays empty only when nobody else free that period can reach it. A far neighbor whose turn comes keeps it by riding, so the Treeline changes who goes as well as how. When all spots are taken, other neighbors keep their regular stroll instead of waiting in a queue. A house being previewed in the builder joins the back of every line, so it only takes a spot the town leaves free and never changes anyone else's day.

Guests leave home shortly after 12:00 or 18:00, follow roads (or ride the Treeline when it saves at least ten minutes), then walk onto the venue lawn. Departures are staggered slightly, so most guests arrive a few minutes early and wait at their spot: sitting on the blanket or in their cinema seat, or standing on the audience lawn, facing the show, with a status such as "Waiting for Jazz under the stars". Zoo visitors and football fans start watching as soon as they arrive, because the animals and the match are already there. Everyone gets home before the next routine starts. Neighbors at work or home remain indoors.
...
`src/lib/events.ts` owns venues, event choices, and schedules; `src/city/venues.ts` draws them. `src/lib/resident-trips.ts` draws the guests and plans their trips, `src/lib/simulation.ts` places everyone at each minute, and `TownEvents.tsx` exposes the daily program through keyboard-accessible buttons. Existing house JSON requires no new fields.
...
Run `npm run check` and `npm run format:check`. Event tests cover UTC rollover, stable selection, reserved plots, attendance limits, road paths, no teleporting at arrival/departure, early guests waiting at their spots, indoor routines, and exact event boundaries. `tests/event-seats.test.ts` fills a synthetic full town of 141 houses for a town year: every seat is filled when enough neighbors can make it, no spot is ever shared or over capacity, every trip keeps to its window and bedtime while rides and parcels never share the tube, seats go round the town, every night owl who can reach the stage before bedtime dances on some nights, a previewed draft never moves anyone, and the draw is the same in every browser language. Verify the stage and green at close zoom and on mobile after artwork changes.

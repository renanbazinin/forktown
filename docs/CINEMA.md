...
Sixteen original films, one minute each. The first six opened the cinema and have been remastered in the style of the ten-film **Starlight Reel** that followed them: the same titles and premises, with fuller stories.
...
Each film is a self-contained module in `src/films/` that exports its pictures, its score, and its title-card look (`FilmModule` in `src/films/types.ts`), registered in `src/films/index.ts`. `src/city/cinema-films.ts` draws whichever film is playing, plus the intermission cards.
